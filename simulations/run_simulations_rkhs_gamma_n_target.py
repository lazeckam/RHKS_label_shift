...
df_results_ipr = pd.DataFrame(np.zeros((N*len(n_target_seq), 9)), 
                              columns=['n_plus', 'n_minus', 'n_target', 'pi_target', 'pi', 'var_n', 'var', 'seed', 'gamma'])
df_results_nrm = pd.DataFrame(np.zeros((N*len(n_target_seq), 9)), 
                              columns=['n_plus', 'n_minus', 'n_target', 'pi_target', 'pi', 'var_n', 'var', 'seed', 'gamma'])
...
for i in tqdm(range(N)):
    for n_target in n_target_seq:
        ...
        df_results_ipr.loc[iter, 'gamma'] = mod.gamma_opt
        df_results_nrm.loc[iter, 'gamma'] =  mod.gamma_opt

        df_results_ipr.to_csv("./results/"+save_as+"/ipr_gamma_numerical_pi_target"+pi_target_name+".csv", index=False)
        df_results_nrm.to_csv("./results/"+save_as+"/nrm_gamma_numerical_pi_target"+pi_target_name+".csv", index=False)
...
for i in tqdm(range(N)):
    for n_target in n_target_seq:
        ...
        df_results_ipr.to_csv("./results/"+save_as+"/ipr_gamma_grid_pi_target"+pi_target_name+".csv", index=False)
        df_results_nrm.to_csv("./results/"+save_as+"/nrm_gamma_grid_pi_target"+pi_target_name+".csv", index=False)
