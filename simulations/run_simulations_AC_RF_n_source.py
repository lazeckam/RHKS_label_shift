...
df_results_ipr = pd.DataFrame(np.zeros((N*len(n_source_seq), 8)), 
                              columns=['n_plus', 'n_minus', 'n_target', 'pi_target', 'pi', 'var_n', 'var', 'seed'])
df_results_nrm = pd.DataFrame(np.zeros((N*len(n_source_seq), 8)), 
                              columns=['n_plus', 'n_minus', 'n_target', 'pi_target', 'pi', 'var_n', 'var', 'seed'])
...
for i in tqdm(range(N)):
    for n_source in n_source_seq:
        ...
        mod = estimator_AC(p_target, p_source_plus, p_source_minus)
        ...
        df_results_ipr.to_csv("./results/"+save_as+"/Bella_RF_pi_target"+pi_target_name+".csv", index=False)
        df_results_nrm.to_csv("./results/"+save_as+"/Forman_RF_pi_target"+pi_target_name+".csv", index=False)
