...
for i in tqdm(range(N)):
    for n_target in n_target_seq:
        ...
        df_results.to_csv("./results/"+save_as+"/Bella_RF_Vaz_pi_target"+pi_target_name+".csv", index=False)
...
for i in tqdm(range(N)):
    for n_target in n_target_seq:
        ...
        mod = estimator_AC_Vaz(p_target, p_source_plus, p_source_minus, g='g.forman.random')
        mod.compute_basic_simulations()
        ...
        df_results.to_csv("./results/"+save_as+"/Forman_RF_Vaz_pi_target"+pi_target_name+".csv", index=False)
